...
git clone [https://github.com/marla22/paymyseat.git](https://github.com/marla22/paymyseat.git)
cd paymyseat
...
docker compose exec payment_api python -m unittest discover -s .
...
infra/
  cloud/
    terraform/          Terraform IaC per provisioning AWS EC2, VPC, Subnet e Security Groups
    ansible/            Ansible playbook per configurazione remota Docker/Docker Compose
.github/
  workflows/
    deploy.yml          Pipeline CI/CD con GitHub Actions per test, build e deploy
...
Il progetto usa credenziali e token, quindi la difesa contro la fuga di segreti è su tre livelli:
    ...
    .gitignore — *.pem, .env, *.tfstate, *.tfvars, .terraform/
